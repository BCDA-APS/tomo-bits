...
class TomoScanDevice(Device):
    ...
    **kwargs
    ...
    sample_out_angle_enable = Cpt(
        EpicsSignal, "SampleOutAngleEnable", string=True, kind="config"
    )
    sample_out_angle = Cpt(EpicsSignal, "SampleOutAngle", kind="config")

    different_flat_exposure = Cpt(
        EpicsSignal, "DifferentFlatExposure", string=True, kind="config"
    )
    ...
    file_plugin_pv_prefix = Cpt(
        EpicsSignal, "FilePluginPVPrefix", string=True, kind="config"
    )

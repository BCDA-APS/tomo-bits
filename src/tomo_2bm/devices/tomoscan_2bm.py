...
class TomoScan2BMDevice(TomoScanHelicalDevice):
    ...
    remote_analysis_dir = Cpt(
        EpicsSignal, "RemoteAnalysisDir", string=True, kind="config"
    )

    # File path configuration
    detector_top_dir = Cpt(EpicsSignal, "DetectorTopDir", string=True, kind="config")
    experiment_year_month = Cpt(
        EpicsSignal, "ExperimentYearMonth", string=True, kind="config"
    )

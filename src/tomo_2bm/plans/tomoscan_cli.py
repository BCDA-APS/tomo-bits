...
def init(args):
    ...
    log.info(
        "For arbitrary scans location custormize (SampleX, SampleY) in {0}".format(
            args.scan_file
        )
    )
    ...
    if ts_pvs["ServerRunning"].get():
        if ts_pvs["ScanStatus"].get(as_string=True) == "Scan complete":
            ...
            if (args.sleep_steps >= 1) and (args.sleep == True):
                log.warning(
                    "running %d x %2.2fs sleep scans", args.sleep_steps, args.sleep_time
                )
                tic = time.time()
                for ii in np.arange(0, args.sleep_steps, 1):
                    ...
                    if (args.sleep_steps + 1) != (ii + 1):
                        if args.in_situ:
                            in_situ_set_value = (
                                args.in_situ_start + (ii) * args.in_situ_step_size
                            )
            ...
            log.error(
                "Server %s is busy. Please run a scan manually first.",
                args.tomoscan_prefix,
            )
    else:
...
def scan(args, ts):
    ...
    if args.scan_type == "Single":
        ...
        log.info(
            "vertical positions (mm): %s",
            np.linspace(start_y, end_y, steps_y, endpoint=False),
        )
        for i in np.linspace(start_y, end_y, steps_y, endpoint=False):
            ...
            log.info(
                "horizontal positions (mm): %s",
                np.linspace(start_x, end_x, steps_x, endpoint=False),
            )
        ...
        if args.scan_type == "Horizontal":
            ...
            log.info(
                "horizontal positions (mm): %s",
                np.linspace(start, end, steps, endpoint=False),
            )
            if flat_field_axis in ("Y") or flat_field_mode == "None":
                pv = "SampleX"
            else:
...
def single_scan(args, ts):
    ...
    if args.testing:
        ...
        ts["StartScan"].put(
            1, wait=True, timeout=360000
        )  # -1 - no timeout means timeout=0
    dtime = (time.time() - tic_01) / 60.0
...
def energy_scan(args, ts):
    ...
    for energy in energies:
        ...
        for k in range(len(pvs1)):
            vals.append(
                vals1[k]
                + (energy - vals1[0]) * (vals2[k] - vals1[k]) / (vals2[0] - vals1[0])
            )
        if args.testing:
            log.warning("testing mode")
        else:
...
def main():
    ...
    cmd_parsers = [
        ...
        (
            ...
            run_horizontal,
            horizontal_param,
            "Run a horizontal tomographic scan",
        ),
        ...
        (
            ...
            run_file,
            file_param,
            "Run a series of scans using the position stored in a configuration file",
        ),
    ]
